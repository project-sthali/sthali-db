...
import sthali_db.clients.postgres


class TestPostgresClient(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        self.client = sthali_db.clients.postgres.PostgresClient("test_path", "test_table")
        self.resource_id: sthali_db.clients.postgres.ResourceId = uuid4()
        self.resource_obj: sthali_db.clients.postgres.ResourceObj = {}
    ...
    async def test_select_many(self) -> None:
        paginate_parameters = sthali_db.clients.postgres.dependencies.PaginateParameters()  # type: ignore
